...
static void settings_init(void) {
    ...
    settings.lru_segmented = false; /* We want 1 LRU, keep this false */
...
void conn_release_items(conn *c) {
    ...
    if (c->item) {
        if (c->item_malloced) {
            //It seems the item is only ever malloc in proxy_process_command()
            //  which we are ignoring. So it should be safe to ignore this/leave it as is
            free(c->item);
            c->item_malloced = false;
        } else {
            item_remove(c->item);
...
item* limited_get_locked(const char *key, size_t nkey, LIBEVENT_THREAD *t, bool do_update, uint32_t *hv, bool *overflow) {
    printf("limited_get_locked called! TODO: is it necessary?\n");
    item *it;
